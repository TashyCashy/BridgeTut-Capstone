...
def create_game(user_id, dealer): 
    ...
    try: 
        ...
        cursor.execute(
            "INSERT INTO games "
            "(user_id, dealer, seq_num, attempt_num, vulnerability, "
            "NS_score, EW_score, date_played) "
...
def get_games_by_date(user_id, selected_date):
    ...
    try:
        cursor.execute(
            "SELECT game_id, dealer, declarer, vulnerability, "
            "NS_score, EW_score, seq_num, attempt_num "
            "FROM games "
            "WHERE user_id = %s AND date_played = %s "
            "ORDER BY seq_num",
